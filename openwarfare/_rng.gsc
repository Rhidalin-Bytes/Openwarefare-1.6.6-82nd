...
rngDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if ( isDefined( eAttacker ) && isPlayer( eAttacker ) && self != eAttacker && sHitLoc != "none" && !eAttacker playerAds() ) {
		...
		if ( ( maps\mp\gametypes\_weapons::isPrimaryWeapon( sWeapon ) || maps\mp\gametypes\_weapons::isPistol( sWeapon ) ) && ( level.scr_rng_enabled == 1 || weaponClass( sWeapon ) != "spread" ) && sMeansOfDeath != "MOD_MELEE" && sMeansOfDeath != "MOD_BAYONET" && !isBipod( sWeapon ) ) {
...
isBipod( sWeapon )
{
	if ( isSubStr( sWeapon, "bipod_prone" ) )
		return true;
	if ( isSubStr( sWeapon, "bipod_crouch" ) )
		return true;
	if ( isSubStr( sWeapon, "bipod_stand" ) )
		return true;
