...
// 82ndAB Changes:	function added: player_is_bleeding() - used for ceasefire to prevent weapon from returning after bandaging
//									splintSelf() Splint broken legs
//									splintTeammate() Splint teammates broken legs
//******************************************************************************
...
{  
  ...
  level.scr_healthsystem_medic_splinting = getdvarx( "scr_healthsystem_medic_splinting", "int", 1, 0, 1 );
...
//82ndAB: Added variables
		self.isSplinting = false;
...
//82ndAB: Added variable
		if ( level.scr_de_break_ankle_on_fall > 0 )
			self.isSplintingTeammate = false;
	}   
...
onPlayerDamaged()
{
	...
	for (;;)
	{
		...
		if ( iDamage > 0 && sMeansOfDeath != "MOD_FALLING" && !self maps\mp\_vehicles::player_is_driver() &&
			   sWeapon != "signal_flare_mp" && sWeapon != "tabun_gas_mp" && sWeapon != "m8_white_smoke_mp" ) 
		{
...
bleedPlayer( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if ( self.health <= 0 ) 
	{
		...
		self maps\mp\_laststand::ensureLastStandParamsValidity();
		self suicide();
	} 
	else 
	{
...
medic()
{
	...
	else if ( isDefined( teammate.ankleBroken ) && teammate.ankleBroken )
	{
		if ( !level.scr_healthsystem_medic_bandaging )
		{
		self thread healTeammate( teammate );
		...
		{
		self thread splintTeammate( teammate );
		}
	}
	else
	{
...
//82ndAB: Vermicious - Splint broken legs
splintSelf()
{     
	self endon("disconnect");
	self endon("death");
	...
	if ( self.ankleFail )
	{
		self iprintln( &"OW_SPLINT_FAIL" );
		return;
	}
	
	if ( isDefined( self.ankleBroken ) && self.ankleBroken  && !self.ankleSplinted ) 
	{
		// If the player is already splinting then we need to cancel the process
		if ( self.isSplinting ) 
		{
			self.isSplinting = false;
			self stopLocalSound( "bandage" );
		...
		else 
		{
			...
			if ( self playerHasBandages() ) 
			{
				...
				self iprintln( &"OW_SPLINT_SELF" );
				
				bandageEnds = openwarfare\_timer::getTimePassed() + level.scr_healthsystem_bandage_time * 1000;
				while ( self.isSplinting && bandageEnds > openwarfare\_timer::getTimePassed() )
					wait (0.05);
				...
				{
					// The necessary time for bandaging has been completed
					self.ankleSplinted = true;
					if ( isDefined(self.bothAnklesBroken ) && self.bothAnklesBroken )
					self thread openwarfare\_speedcontrol::setModifierSpeed( "_damageeffect_fall", level.scr_de_break_ankle_on_fall_percent );
					else
					{
						self thread openwarfare\_speedcontrol::setModifierSpeed( "_damageeffect_fall", level.scr_de_break_ankle_on_fall_percent - 30 );
					...
					self setClientDvar( "ui_bandages_qty", self.totalBandages );
					self stopLocalSound( "bandage" );
					self iprintln( &"OW_SPLINT_SUCCESS" );
					self thread openwarfare\_damageeffect::strapAnkle();
...
//82ndAB: Vermicious - Splint teammates broken legs
splintTeammate( teammate )
{
	if ( !level.scr_healthsystem_medic_splinting )
		return; 
  
	//If we are already splinting the teammate then we need to stop    
	if ( isDefined( self.isSplintingTeammate ) && self.isSplintingTeammate )
	{
		self.isSplintingTeammate = false;
	}
	else
	{ 
		self.isSplintingTeammate = true;
    
		self iprintln( &"OW_MEDIC_SELF_SPLINT", teammate );
		teammate iprintln( &"OW_MEDIC_TEAMMATE_SPLINT", self );
		...
		bandageEnds = openwarfare\_timer::getTimePassed() + level.scr_healthsystem_bandage_time * 1000;
		while ( isAlive( teammate ) && self.isSplintingTeammate && bandageEnds > openwarfare\_timer::getTimePassed() )
		{
			if ( ( isDefined( self.lastStand ) && self.lastStand ) || ( isDefined( teammate.lastStand ) && teammate.lastStand ) ) 
				break; 
			wait (0.05);
		...
		if ( self.isSplintingTeammate ) 
		{    
			teammate.ankleSplinted = true;  
			if ( isDefined(teammate.bothAnklesBroken ) && teammate.bothAnklesBroken )
				teammate thread openwarfare\_speedcontrol::setModifierSpeed( "_damageeffect_fall", level.scr_de_break_ankle_on_fall_percent );
			else
				teammate thread openwarfare\_speedcontrol::setModifierSpeed( "_damageeffect_fall", level.scr_de_break_ankle_on_fall_percent - 30 );
			self.isSplintingTeammate = false;
	...
	if ( level.scr_healthsystem_bleeding_icon && isDefined( self.hud_healthsystem_icon ) && !teammate.ankleBroken )
		teammate showHealingStatus( false );
