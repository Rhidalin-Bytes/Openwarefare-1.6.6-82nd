...
init()
{
	...
	level.scr_anti_camping_enable = getdvarx( "scr_anti_camping_enable", "int", 0, 0, 3 );
...
monitorPlayerCamping()
{
	...
	for (;;)
	{
		...
		if ( ( !isDefined( self.interacting_with_objective ) || !self.interacting_with_objective ) && ( !isDefined( self.isVIP ) || !self.isVIP ) && ( !isDefined( self.isDefusing ) || !self.isDefusing ) && ( !isDefined( self.isPlanting ) || !self.isPlanting ) && ( level.gametype != "ftag" || !self.freezeTag["frozen"] ) ) {
			...
			if ( distanceMoved < level.scr_anti_camping_distance && !self isInVehicle() ) {
				...
				if ( level.scr_anti_camping_enable == 1 ||
					 ...
					 ( level.scr_anti_camping_enable == 3 && !maps\mp\gametypes\_weapons::isSniper( self getCurrentWeapon() ) && !maps\mp\gametypes\_weapons::isBolt( self getCurrentWeapon() ) ) ) {
