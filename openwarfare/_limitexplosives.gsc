...
init()
{
	...
	level.scr_limit_planted_satchels = getdvarx( "scr_limit_planted_satchels", "int", 0, 0, 50 );
	level.scr_limit_planted_bettys = getdvarx( "scr_limit_planted_bettys", "int", 0, 0, 50 );

	// Check if we need to stay here or not
	if ( ( level.scr_limit_planted_satchels == 0 && level.scr_limit_planted_bettys == 0 ) || !level.teamBased )
		return;
...
onPlayerSpawned()
{
	...
	if ( level.scr_limit_planted_satchels > 0 )
		self thread monitorPlantedExplosives( level.scr_limit_planted_satchels, "satchel_charge_mp" );
	if ( level.scr_limit_planted_bettys > 0 )
		self thread monitorPlantedExplosives( level.scr_limit_planted_bettys, "mine_bouncing_betty_mp" );
}
