...
onPlayerSpawned()
{
	...
	if ( self hasPerk( "specialty_longersprint" ) ) {
		maxSprintTime = level.scr_player_sprinttime * 1.4 * 1000;
	} else {
...
monitorWalkWithoutADS()
{
	...
	for (;;)
	{
		...
		if ( self getVelocity() != 0 && self getStance() == "stand" && self depthInWater() < 35 && self isOnGround() && !self.fcs["sprinting"] && !self playerAds() ) {
