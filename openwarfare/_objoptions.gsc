...
getSafeZoneGametype()
{
  ...
  switch( gametype )
  {
    ...
    case "sd":
      ...
      break;      
    case "twar":
      self thread objSafeZones1( "twar" );
      break;
    case "koth":
...
objSafeZones1( gametype )
{
  ...
  else if ( gametype == "sab" )
  {
    ...
    objZones = getEntArray( "flag_primary_linked", "targetname" );
  else
  {
...
allowDefenderExplosiveDestroy()
{
  ...
  if ( isDefined( level.multiBomb ) && level.multiBomb )
	return;
  ...
  {     
    ...
    if ( self.destroyingExplosive )
    {
      ...
      if( level.scr_sd_show_briefcase )
      {
        self detach( "weapon_explosives", "tag_inhand" ); 
