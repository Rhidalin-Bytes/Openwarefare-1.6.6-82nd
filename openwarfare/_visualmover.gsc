...
getConfigurationCount()
{
	...
	//Make sure to check for tank dvars
	dvarList[dvarList.size] = level.mapname + "_move_" + level.vm_gametype + "_stock_tanks";
	dvarList[dvarList.size] = level.mapname + "_move_" + level.vm_gametype + "_spawn_tanks";
...
moveThePieces()
{
	switch( level.vm_gametype )
	{
		...
		case "sab":
					if ( level.vm_gametype == "sab" )
					{
						...
						thread moveFlags( level.mapname + "_move_ch_flags_" + game["mover_config_current"] );
					
					moveSpawns( "mp_sab_spawn_allies", level.mapname + "_move_" + level.vm_gametype + "_allies_spawns_" + game["mover_config_current"] );
					moveSpawns( "mp_sab_spawn_axis", level.mapname + "_move_" + level.vm_gametype + "_axis_spawns_" + game["mover_config_current"] );
					moveSpawns( "mp_sab_spawn_allies_start", level.mapname + "_move_" + level.vm_gametype + "_allies_start_spawns_" + game["mover_config_current"] );
					moveSpawns( "mp_sab_spawn_axis_start", level.mapname + "_move_" + level.vm_gametype + "_axis_start_spawns_" + game["mover_config_current"] );
			break;
		case "sd":
		case "re":
					if ( level.vm_gametype == "sd" )
		...
		case "twar":
					...
					if ( level.vm_gametype == "ctf" )
					{
						moveSpawns( "mp_ctf_spawn_allies", level.mapname + "_move_ctf_allies_spawns_" + game["mover_config_current"] );
						moveSpawns( "mp_ctf_spawn_axis", level.mapname + "_move_ctf_axis_spawns_" + game["mover_config_current"] );
					}
					else
	...
	//Move stock and spawn tanks
	thread moveStockTanks( level.mapname + "_move_" + level.vm_gametype + "_stock_tanks_" + game["mover_config_current"] );
	spawnTanks( level.mapname + "_move_" + level.vm_gametype + "_spawn_tanks_" + game["mover_config_current"] );
...
moveObjectives( dvar )
{
	...
	for ( idx = 0; idx < coords.size; idx++ )
	{
		...
		if ( level.vm_gametype != "re" )
		{
			...
			//Move defuse trigger (do we have to do this?)
			/*if ( level.vm_gametype == "sd" )
			{
				defuseTrig = getEnt( visuals[0].target, "targetname" );
				defuseTrig.origin = newOrigin;
				defuseTrig.angles = newAngles;
			}*/
		}
...
moveFlags( dvar )
{
	...
	{
		triggers = getEntArray( "ctf_flag_pickup_trig", "targetname" );
		
		if ( triggers.size == 0 )
			return;
		
		zones = getEntArray( "ctf_flag_zone_trig", "targetname" );
		models = [];
		models[0] = getEnt( "ctf_flag_allies", "targetname" );
		models[1] = getEnt( "ctf_flag_axis", "targetname" );
		...
		for ( idx = 0; idx < coords.size; idx++ )
		{
			...
			if ( triggers[curFlag].script_team == "allies" )
			{
				models[0].origin = newOrigin;
				models[0].angles = newAngles;
			}
			else
			{
				models[1].origin = newOrigin;
				models[1].angles = newAngles;
			}
		}	
	}	
	else if ( level.vm_gametype == "dom" )
	{
	...
	{
		triggers = getEntArray( "flag_primary_linked", "targetname" );
		
		if ( triggers.size == 0 )
		return;
	
		descriptors = getEntArray( "twar_flag_descriptor", "targetname" );
		...
		for ( idx = 0; idx < coords.size; idx++ )
		{
			if ( coords[idx][0] == 0 )
				...
				sCoord3[sCoord3.size] = coords[idx];
			else if ( coords[idx][0] == 4 )
				sCoord4[sCoord4.size] = coords[idx];
		}
		//Select the random bombsites 
		coords = [];
		if ( sCoord0.size > 0 )
		...
		if ( sCoord2.size > 0 )	
			coords[coords.size] = sCoord2[randomint(sCoord2.size)];
		if ( sCoord3.size > 0 )	
			coords[coords.size] = sCoord2[randomint(sCoord3.size)];
		if ( sCoord4.size > 0 )	
			coords[coords.size] = sCoord2[randomint(sCoord4.size)];	
...
moveRadios( dvar )
{
	...
	for ( idx = 0; idx < coords.size; idx++ )
	{
		...
		hardPoints[curRadio] linkTo( visuals[2] );
				
		//Link together everything for the move - makes it so much easier
		for ( idx2 = 0; idx2 < visuals.size; idx2++ )
		{
			if ( idx2 != 2 )
				visuals[idx2] linkTo( visuals[2] );
		}
...
/*
*	Function: 		moveStockTanks <dvar> )
*	Purpose:		To delete stock tanks and spawn new tanks at the locations given in <dvar>
*	Pre-Condition:  The dvar containing the new coordinates
*	Post-Condition:	None.
*/
moveStockTanks( dvar )
{
	...
	temp = [];	
	for ( idx = 0; idx < tanks.size; idx++ )
	{
		if ( tanks[idx] maps\mp\_vehicles::vehicle_is_tank() )
			temp[temp.size] = tanks[idx];
	}	
	tanks = temp;
	
	//Check again just in case all the script_vehicles were not tanks
	if ( tanks.size == 0 )
		return;
	...
	for ( idx = 0; idx < coords.size; idx++ )
	{		
		curTank = coords[idx][0];
	
		if ( curTank >= tanks.size || curTank < 0 )
			continue;
	
		origin = ( coords[idx][1], coords[idx][2], coords[idx][3] );
		angles = ( 0, coords[idx][4], 0 );
		modelname = tanks[curTank].model;
		targetname = tanks[curTank].targetname;
		vehicletype = tanks[curTank].vehicletype;
		destructibledef = tanks[curTank].destructibledef;
		...
		vehicle.destructibledef = destructibledef;
		vehicle.health = tanks[curTank].initial_state.health;
		
		vehicle maps\mp\_vehicles::init_vehicle();
	
		vehicle maps\mp\_vehicles::vehicle_telefrag_griefers_at_position( origin );	
...
/*
*	Function: 		spawnTanks <dvar> )
*	Purpose:		To spawn new tanks at the locations given in <dvar>
*	Pre-Condition:  The dvar containing the new coordinates
*	Post-Condition:	None.
*/
spawnTanks( dvar )
{
	...
	for ( idx = 0; idx < coords.size; idx++ )
	{
		origin = ( coords[idx][1], coords[idx][2], coords[idx][3] );
		angles = ( 0, coords[idx][4], 0 );
		vehicle = coords[idx][5];
		
		curTank = coords[idx][0];
	
		if ( curTank >= coords.size || curTank < 0 )
			continue;
		
		tank = getTankInfo( origin, angles, vehicle );
		
		spawnTank = SpawnVehicle( tank.modelname, tank.targetname, tank.vehicletype, tank.origin, tank.angles, tank.destructibledef );

		spawnTank.vehicletype = tank.vehicletype;
		spawnTank.destructibledef = tank.destructibledef;
		spawnTank.health = level.scr_veh_health_tank;
		spawnTank maps\mp\_vehicles::init_vehicle();
		spawnTank maps\mp\_vehicles::vehicle_telefrag_griefers_at_position( tank.origin );			
	}	
}
...
getParsedCoords( coordString )
{
	...
	for ( idx = 0; idx < coordSets.size; idx++ )
	{
		...
		if ( coordParams.size < 5 || coordParams.size > 6 )
			continue;
		...
		if ( coordParams.size == 6 )
		{
			if ( isSubStr( coordParams[5], "panzer4_mp" ) || isSubStr( coordParams[5], "t34_mp" ) )
				coords[5] = coordParams[5];
		}
...
/*
*	Function: 		getTankInfo( <origin>, <angles>, <vehicle> )
*	Purpose:		To create information necessary for spawning a tank based on <vehicle>
*	Pre-Condition:  Need the tanks spawning origin and angle, and which type of tank it will be. 
*	Post-Condition: Returns information fields for spawning a tank.
*/
getTankInfo( origin, angles, vehicle )
{
	info = spawnStruct();
	if ( isSubStr( vehicle, "panzer4_mp" ) )
	{
		info.origin = origin;
		info.angles = angles;
		info.modelname = "veh_ger_panzer_mp_dpm_main";
		info.vehicletype = vehicle;
		info.targetname = "destructible";
		if ( isSubStr( vehicle, "flm" ) )
			info.destructibledef = "vehicle_ger_Panzer_flm_mp_ddef";
		else
			info.destructibledef = "vehicle_ger_Panzer_mg_mp_ddef";
	}
	else
	{
		info.origin = origin;
		info.angles = angles;
		info.modelname = "veh_t34_mp_dpm_main";
		info.vehicletype = vehicle;
		info.targetname = "destructible";
		if ( isSubStr( vehicle, "flm" ) )
			info.destructibledef = "vehicle_rus_T34_mp_flm_ddef";
		else
			info.destructibledef = "vehicle_rus_t34_mp_ddef";
	}
