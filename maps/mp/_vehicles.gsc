...
init()
{
	...
	level.scr_veh_respawntimemin = getdvarx( "scr_veh_respawntimemin", "int", 50, 10, 300 );
	level.scr_veh_respawntimemax = getdvarx( "scr_veh_respawntimemax", "int", 90, level.scr_veh_respawntimemin, 300 );
	...
	level.scr_veh_health_tank = getdvarx( "scr_veh_health_tank", "int", 1350, 100, 2700 );
...
init_vehicle()
{
	...
	self.maxhealth = level.scr_veh_health_tank;
...
player_is_occupant_invulnerable( sMeansOfDeath )
{
	if ( !isDefined( level.vehicle_drivers_are_invulnerable ) )
		level.vehicle_drivers_are_invulnerable = true;
...
player_leave_vehicle_cleanup_t( vehicle )
{
	...
	if( isDefined( self.lastWeapon ) && self.lastWeapon != currentWeapon && self.lastWeapon != "none" )
...
vehicle_is_tank()
{
	return self.vehicletype == "sherman_mp"
		...
		|| self.vehicletype == "panzer4_mp_flm"
		|| self.vehicletype == "t34_mp_flm"
...
respawn_vehicle( respawn_parameters )
{	
	seconds = RandomFloatRange( level.scr_veh_respawntimemin, level.scr_veh_respawntimemax + 1 );
	wait seconds;
