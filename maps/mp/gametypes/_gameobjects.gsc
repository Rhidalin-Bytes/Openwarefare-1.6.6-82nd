...
// 82ndAB changes: function added: _ceasefire() - disable weapons indefinitely
//******************************************************************************
...
main(allowed)
{
	...
	vehicles_enabled = getdvarx( "scr_vehicles_enabled", "int", 1, 0, 1 );
	...
	{
		/*/ Print map assets
		toLog = "Objectname: "+entities[entity_index].script_gameobjectname;
		if ( isDefined( entities[entity_index].targetname ) )
			toLog += " -> Targetname: " + entities[entity_index].targetname;

		if (isDefined( entities[entity_index]].classname ) )
			toLog += " -> Classname: " + entities[entity_index].classname;
...
useHoldThink( player )
{
	...
	if ( isDefined( useWeapon ) )
	{
		...
		player thread attachUseModel( "weapon_explosives", "tag_inhand", true );
	}
	else
	{
	...
	while ( isAlive( self ) && object.inUse && !level.gameEnded )
	{
		if ( lastRate != object.useRate || lastTimeoutStatus != level.inTimeoutPeriod )
		{
			...
			if ( object.decayProgress && self.pers["team"] != object.claimTeam )
			{
				...
				{
				if ( level.inTimeoutPeriod )
					self.proxBar updateBar( object.curProgress / object.useTime , 0 );
				else
					self.proxBar updateBar( object.curProgress / object.useTime , (1000 / object.useTime) * ( object.useRate * -1 ) );
				}
			}
			...
			{
				if ( level.inTimeoutPeriod )
					self.useBar updateBar( object.curProgress / object.useTime, 0 );
				else
