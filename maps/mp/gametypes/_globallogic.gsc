...
init()
{
	...
	setDvar( "_ModVer", "v1.6.5.UD", true );
	...
	/*if ( !isSubStr( toLower(getDvar("sv_referencedFFNames")), "mods/mp_openwarfare/mod" ) ) {
...
{	
	...
	if ( level.scr_match_readyup_period == 1 ) {
		...
		game["matchReadyUpText"] setPoint( "CENTER", "CENTER", 0, -65 );
	...
	countTime = int( level.prematchPeriodEnd );
...
endGame( winner, endReasonText )
{
	...
	for ( index = 0; index < level.players.size; index++ )
	{
		...
		if ( level.rankedMatch && level.scr_endofgame_stats_enable == 0 ) {
			level.players[index] setClientDvars(
				"ns_n", nemesis,
...
{
	return getdvarx( level.timeLimitDvar, "float", level.timeLimit, level.timeLimitMin, level.timeLimitMax );
}
...
gameTimer()
{
	...
	if ( level.scr_enable_music && level.hardcoreMode == 0 )
...
waitAndSpawnClient( timeAlreadyPassed )
{
	...
	waveBased = (getdvarx( "scr_" + level.gameType + "_waverespawndelay", "float", 0, 0, 300 ) > 0);
	if ( level.scr_player_forcerespawn == 0 && self.hasSpawned && !waveBased && !self.wantSafeSpawn )
	{
...
checkRoundSwitch()
{
	...
	if ( game["roundsplayed"] % level.roundswitch == 0 )
	{
		...
		if( level.scr_switch_teams_at_halftime == 1  )
		{
			...
			// Switch internal CTF scores also
			if ( level.gametype == "ctf" ) {
				tempscores = game["ctf_teamscore"]["allies"];
				game["ctf_teamscore"]["allies"] = game["ctf_teamscore"]["axis"];
				game["ctf_teamscore"]["axis"] = tempscores;
			}
...
//	self thread fakeLag();
	...
	if ( level.numLives )
	{
		self setClientDvars("cg_deadChatWithDead", "1",
							"cg_deadChatWithTeam", getdvarx( "scr_enable_deadchat", "int", 0, 0, 1 ),
	...
	else
	{
		self setClientDvars("cg_deadChatWithDead", "0",
							"cg_deadChatWithTeam", getdvarx( "scr_enable_deadchat", "int", 1, 0, 1 ),
...
Callback_PlayerDamage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	...
	if ( level.gametype == "ftag" && self.freezeTag["frozen"] ) {
		self thread maps\mp\gametypes\ftag::playerFrozenDamaged( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime );
		return;
	}
...
// Check if the player is spawn protected and there's an attacker (there's no attacker if the player falls)
	...
	if ( sWeapon == "none" && isDefined( eInflictor ) )	{
		...
		if ( (!level.scr_vehicle_damage_enable && (sWeapon == "destructible_car")) ||  (!level.scr_barrel_damage_enable && sWeapon == "explodable_barrel"))
		{
			return;
		}
	...
	// create a class specialty checks; CAC:bulletdamage, CAC:armorvest
	if ( !level.rankedMatch ) {
		iDamage = maps\mp\gametypes\_class_unranked::cac_modified_damage( self, eAttacker, iDamage, sMeansOfDeath, sWeapon, eInflictor );
	} else {
		iDamage = maps\mp\gametypes\_class::cac_modified_damage( self, eAttacker, iDamage, sMeansOfDeath, sWeapon, eInflictor );
	}
			...
			else
			{
				...
				if ( isAlive( attacker ) )
				{
					if ( !isDefined( eInflictor ) || !isDefined( eInflictor.requiredDeathCount ) || attacker.deathCount == eInflictor.requiredDeathCount ) {
						...
						switch ( sWeapon ) {
							case "artillery_mp":
								if ( level.scr_artillery_kills_toward_streak == 1 ) {
									attacker.cur_kill_streak++;
									streakGiven = true;
								}
								break;
							case "dog_bite_mp":
								if ( level.scr_dogs_kills_toward_streak == 1 ) {
									attacker.cur_kill_streak++;
									streakGiven = true;
								}
								break;
				...
				if ( level.teamBased && attacker.pers["team"] != "spectator")
				{
					// dog score for team
					if( isai(Attacker) )
					        giveTeamScore( ScoreOfPlayer["type"], attacker.aiteam, attacker, self );
					else
...
createDeadBody( iDamage, sMeansOfDeath, sWeapon, sHitLoc, vDir, vAttackerOrigin, deathAnimDuration, eInflictor, ragdoll_jib, body )
{
	...
	if ( !isDefined( self.switching_teams ) && level.scr_hud_show_death_icons )
...
getPerks( player )
{
	...
	if ( isPlayer( player ) && !level.oldschool )
	{
		...
		if ( !level.rankedMatch || level.gametype == "gg" ) {
			if ( !isdefined( player.pers["isBot"] ) && isDefined( player.pers ) && isDefined( player.class ) && isDefined( player.pers[player.class]["loadout_perk1"] ) ) {
				perks[0] = player.pers[player.class]["loadout_perk1"];
				perks[1] = player.pers[player.class]["loadout_perk2"];
				perks[2] = player.pers[player.class]["loadout_perk3"];
				perks[3] = player.pers[player.class]["loadout_perk4"];
			}
		} else if ( level.onlineGame && !isdefined( player.pers["isBot"] ) && ( isSubstr( player.curClass, "CLASS_CUSTOM" ) || isSubstr( player.curClass, "CLASS_PRESTIGE" ) ) && isdefined(player.custom_class) ) {
...
playLeaderDialogOnPlayer( dialog, team )
{
	...
	if( level.allowAnnouncer && isDefined( game["dialog"][dialog] ) ) {
