...
main()
{
	...
	level.scr_tdm_forcestartspawns = getdvarx( "scr_tdm_forcestartspawns", "int", 0, 0, 1 );
	...
	// Check if we need to run the unified spawn functions
	level.onSpawnPlayerUnified = undefined;
	if ( level.scr_tdm_forcestartspawns == 0 && level.scr_unified_spawns_enable == 1 ) {
...
onSpawnPlayer()
{
	...
	if ( level.inGracePeriod || level.scr_tdm_forcestartspawns )
	{
