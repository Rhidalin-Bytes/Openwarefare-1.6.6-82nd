...
onPrecacheGameType()
{
	...
	if ( game["attackers"] == "allies" ) {
		...
		if ( game["allies"] == "marines" ) {
			game[level.gameType]["extraction_base"] = loadFX( "misc/ui_flagbase_blue" );
		} else {
			game[level.gameType]["extraction_base"] = loadFX( "misc/ui_flagbase_orange" );
		}
	} else {
...
onDrop( player )
{
	...
	if ( isDefined( player ) ) {
		...
		thread printAndSoundOnEveryone( game["defenders"], game["attackers"], &"OW_RE_DROPPED_BY", &"OW_RE_DROPPED_BY", "mp_obj_taken", "mp_enemy_obj_taken", player, player, self.longname );
	
	} else {
		// Play sound and show the proper message
		thread printAndSoundOnEveryone( game["defenders"], game["attackers"], &"OW_RE_DROPPED", &"OW_RE_DROPPED", "mp_obj_taken", "mp_enemy_obj_taken", self.longname );
...
onReset( player )
{
	thread printAndSoundOnEveryone( game["defenders"], game["attackers"], &"OW_RE_RETURNED", &"OW_RE_RETURNED", "mp_obj_returned", "mp_obj_returned", self.longname );
