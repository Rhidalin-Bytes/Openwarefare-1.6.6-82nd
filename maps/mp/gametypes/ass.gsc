...
onPrecacheGameType()
{
	...
	switch ( game[game["attackers"]] ) {
		case "marines":
			game[level.gameType]["extraction_base_effect"] = loadFX( "misc/ui_flagbase_blue" );
			break;
		case "russian":
			game[level.gameType]["extraction_base_effect"] = loadFX( "misc/ui_flagbase_orange" );
			break;
		case "japanese":
	...
	precacheModel( "char_ger_volks_head1_1" );
}
...
pickVIP()
{
	...
	level.playerVIP attach( "char_ger_volks_head1_1", "", true );	
	...
	// Set the Light Gunner speed and reset the health if set
	level.playerVIP thread openwarfare\_speedcontrol::setBaseSpeed( getdvarx( "class_lightgunner_movespeed", "float", 1.0, 0.5, 1.5 ) );
