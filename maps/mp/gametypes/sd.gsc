...
onPickup( player )
{
	...
	if ( level.scr_sd_scoreboard_bomb_carrier == 1 && player.pers["team"] == game["attackers"] ) { 
		player.statusicon = "hud_status_bomb";
	}
	
	if ( player.pers["team"] == game["defenders"] && level.scr_sd_allow_defender_explosivedestroy ) 
    player iprintln( &"OW_DESTROY_EXPLOSIVES" );
	
	if ( isDefined( player ) && player.pers["team"] == game["attackers"] )	
    self maps\mp\gametypes\_gameobjects::set3DIcon( "friendly", "waypoint_defend" );

	if ( !level.bombDefused && player.pers["team"] == game["attackers"] ) 
	{
