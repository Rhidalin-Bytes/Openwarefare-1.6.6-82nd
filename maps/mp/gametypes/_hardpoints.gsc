...
init()
{
	// Get module's dvars
	// General dvars
	level.scr_game_forceradar = getdvarx( "scr_game_forceradar", "int", 0, 0, 1 );
	...
	// Radar related dvars
	level.scr_hardpoint_allow_radar = getdvarx( "scr_hardpoint_allow_radar", "int", 1, 0, 1 );
	level.scr_hardpoint_radar_streak = getdvarx( "scr_hardpoint_radar_streak", "int", 3, 1, 50 );
	level.scr_radar_view_time = getdvarx( "scr_radar_view_time", "float", 15, 1, 120 );
	level.scr_announce_enemy_radar_online = getdvarx( "scr_announce_enemy_radar_online", "int", 1, 0, 1 );

	// Artillery related dvars
	level.scr_hardpoint_allow_artillery = getdvarx( "scr_hardpoint_allow_artillery", "int", 1, 0, 1 );
	level.scr_hardpoint_artillery_streak = getdvarx( "scr_hardpoint_artillery_streak", "int", 5, 1, 50 );
	level.scr_artillery_hardpoint_interval = getdvarx( "scr_artillery_hardpoint_interval", "float", 0, 0, 600 );
	level.scr_announce_enemy_artillery_inbound = getdvarx( "scr_announce_enemy_artillery_inbound", "int", 1, 0, 1 );
	level.scr_artillery_kills_toward_streak = getdvarx( "scr_artillery_kills_toward_streak", "int", 1, 0, 1 );
	level.scr_artillery_delay = getdvarx( "scr_artillery_delay", "int", 0, 0, 30 );

	// Dogs related dvars
	level.scr_hardpoint_allow_dogs = getdvarx( "scr_hardpoint_allow_dogs", "int", 1, 0, 1 );
	level.scr_hardpoint_dogs_streak = getdvarx( "scr_hardpoint_dogs_streak", "int", 7, 1, 50 );
	level.scr_dogs_hardpoint_interval = getdvarx( "scr_dogs_hardpoint_interval", "float", 0, 0, 600 );
	level.scr_announce_enemy_dogs_inbound = getdvarx( "scr_announce_enemy_dogs_inbound", "int", 1, 0, 1 );
	level.scr_dogs_kills_toward_streak = getdvarx( "scr_dogs_kills_toward_streak", "int", 1, 0, 1 );
	level.scr_dogs_delay = getdvarx( "scr_dogs_delay", "int", 0, 0, 30 );
	...
	priority = 0;
	if ( level.scr_hardpoint_allow_radar )
	{
		level.hardpointItems["radar_mp"] = priority;
		priority++;
	}
	
	if ( level.scr_hardpoint_allow_artillery )
	{
		level.hardpointItems["artillery_mp"] = priority;
		priority++;
	}
	
	if ( level.scr_hardpoint_allow_dogs )
	{
	...
	level.radarViewTime = level.scr_radar_view_time; // time radar remains active
...
doArtillery(origin, owner, team, ownerDeathCount )
{
	...
	if ( level.teambased )
	{
		...
		maps\mp\gametypes\_globallogic::leaderDialog( "artillery_inbound", team );
		if ( level.scr_announce_enemy_artillery_inbound == 1 ) {
			maps\mp\gametypes\_globallogic::leaderDialog( "enemy_artillery_inbound", level.otherTeam[team] );
		}
	...
	else
	{
		...
		// Check if we should announce a coming artillery to the enemy
		if ( level.scr_announce_enemy_artillery_inbound == 1 ) {
			maps\mp\gametypes\_globallogic::leaderDialog( "enemy_artillery_inbound", undefined, undefined, ownerarray );
		}
...
giveHardpointItemForStreak()
{
	...
	if ( actualstreak && level.scr_game_hardpoints_cycle == 1 ) {
		...
		if ( level.scr_hardpoint_allow_radar == 1 && level.scr_hardpoint_radar_streak > cyclestreak && level.scr_game_forceradar == 0 )
			cyclestreak = level.scr_hardpoint_radar_streak;

		if ( level.scr_hardpoint_allow_artillery == 1 && level.scr_hardpoint_artillery_streak > cyclestreak )
			cyclestreak = level.scr_hardpoint_artillery_streak;

		if ( level.scr_hardpoint_allow_dogs == 1 && level.scr_hardpoint_dogs_streak > cyclestreak )
			cyclestreak = level.scr_hardpoint_dogs_streak;
	...
	// Check if radar is active and if we have reached the kill streak for it
	if ( level.scr_hardpoint_allow_radar == 1 && streak == level.scr_hardpoint_radar_streak && level.scr_game_forceradar == 0 )
		self giveHardpoint( "radar_mp", actualstreak );

	// Check if Artillery is active and if we have reached the kill streak for it
	else if ( level.scr_hardpoint_allow_artillery == 1 && streak == level.scr_hardpoint_artillery_streak )
		self giveHardpoint( "artillery_mp", actualstreak );

	// Check if Dogs is active and if we have reached the kill streak for it
	else if ( level.scr_hardpoint_allow_dogs == 1 && streak == level.scr_hardpoint_dogs_streak )
		self giveHardpoint( "dogs_mp", actualstreak );
...
giveHardpoint( hardpointType, streak )
{
	...
	self maps\mp\gametypes\_hardpoints::giveHardpointItem( hardpointType, undefined, streak );
}
...
giveHardpointItem( hardpointType, do_not_update_death_count, streak )
{
	if ( level.gameEnded )
		return;
...
//			case "kamikaze_mp":
				...
				{	
					...
					// without this we will get XP only (was missing in orginal ta code :)
...
triggerHardPoint( hardpointType )
{
	...
	else if ( hardpointType == "artillery_mp" )
	{
		...
		// Check if we need to wait for certain interval to use the artillery again
		if ( level.scr_artillery_hardpoint_interval > 0 ) {
			if ( level.teambased ) {
				// Get the time of the last airstrike for the team calling this airstrike
				team = self.pers["team"];
				if ( team == "allies" ) {
					if ( isDefined( level.allies_last_artillery ) ) {
						last_artillery = level.allies_last_artillery;
					} else {
						last_artillery = 0;
					}
				} else {
					if ( isDefined( level.axis_last_artillery ) ) {
			...
			if ( last_artillery > 0 && ( currentTime - last_artillery ) < level.scr_artillery_hardpoint_interval ) {
				availableIn = int(level.scr_artillery_hardpoint_interval - ( currentTime - last_artillery ));
				self iPrintLnBold( &"OW_ARTILLERY_AVAILABLEIN", availableIn );
				return false;
			}
		}
	...
	else if ( hardpointType == "dogs_mp" )
	{
		...
		// Check if we need to wait an interval to use dogs again
		if ( level.scr_dogs_hardpoint_interval > 0 ) {
			if ( level.teambased ) {
				// Get the last time the Helicopter was used by this team
				team = self.pers["team"];
				if ( team == "allies" ) {
					if ( isDefined( level.allies_last_dogs ) ) {
						last_dogs = level.allies_last_dogs;
					} else {
						last_dogs = 0;
					}
				} else {
					if ( isDefined( level.axis_last_dogs ) ) {
						last_dogs = level.axis_last_dogs;
					} else {
						last_dogs = 0;
					}
				}
				...
				if ( isDefined( self.pers["last_dogs"] ) ) {
					last_dogs = self.pers["last_dogs"];
				} else {
					last_dogs = 0;
				}
			}
			...
			currentTime = openwarfare\_timer::getTimePassed() / 1000;
			if ( last_dogs > 0 && ( currentTime - last_dogs ) < level.scr_dogs_hardpoint_interval ) {
				availableIn = int(level.scr_dogs_hardpoint_interval - ( currentTime - last_dogs ));
				self iPrintLnBold( &"OW_DOGS_AVAILABLEIN", availableIn );
				return false;
			}
		}
		...
		if ( level.teambased )
		{
			...
			if ( level.scr_announce_enemy_dogs_inbound == 1 ) {
		...
		else
		{
			...
			// Check if we should announce the dogs to the enemy
			if ( level.scr_announce_enemy_dogs_inbound == 1 ) {
		...
		if ( level.teambased ) {
			team = self.pers["team"];
			if ( team == "allies" )
				level.allies_last_dogs = openwarfare\_timer::getTimePassed() / 1000;
			else
				level.axis_last_dogs = openwarfare\_timer::getTimePassed() / 1000;
		} else {
			self.pers["last_dogs"] = openwarfare\_timer::getTimePassed() / 1000;
		}
...
RadarAcquiredPrintAndSound( team, otherteam, callingPlayer, numseconds )
{
	...
	else
	{
		...
		if( getTime() - level.radarTimers[team] > 30000 )
		{
			...
			if ( level.scr_announce_enemy_radar_online == 1 ) {
