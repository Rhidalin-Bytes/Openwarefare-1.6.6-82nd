...
{	
	...
	if ( game["switchedsides"] ) {
		level.spawn_axis_start = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_dom_spawn_allies_start" );
		level.spawn_allies_start = maps\mp\gametypes\_spawnlogic::getSpawnpointArray( "mp_dom_spawn_axis_start" );
	} else {
...
updateDomScores()
{
	...
	while ( !level.gameEnded )
	{
		...
		if ( isDefined( level.securedAllBonusTime ) && level.securedAllBonusTime <= maps\mp\gametypes\_globallogic::getTimePassed() ) {
			...
			level.securedAllBonusTime = maps\mp\gametypes\_globallogic::getTimePassed() + level.scr_dom_secured_all_bonus_time * 1000;
		}
