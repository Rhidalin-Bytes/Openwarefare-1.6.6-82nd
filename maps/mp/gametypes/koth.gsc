...
//		maps\mp\gametypes\_globallogic::leaderDialog( "move_to_new" );
		...
		if ( level.hqSpawnTime )
		{
			...
			finishWait = maps\mp\gametypes\_globallogic::getTimePassed() + level.hqSpawnTime * 1000;
			while ( finishWait > maps\mp\gametypes\_globallogic::getTimePassed() ) {
				wait (0.05);
				if ( level.inTimeoutPeriod ) {
					timerDisplay["allies"].alpha = 0;
					timerDisplay["axis"  ].alpha = 0;
					xWait( 0.1 );
					timerDisplay["allies"] setTimer( ( finishWait - maps\mp\gametypes\_globallogic::getTimePassed() ) / 1000 );
					timerDisplay["axis"  ] setTimer( ( finishWait - maps\mp\gametypes\_globallogic::getTimePassed() ) / 1000 );
					timerDisplay["allies"].alpha = 1;
					timerDisplay["axis"  ].alpha = 1;
				}
			}
