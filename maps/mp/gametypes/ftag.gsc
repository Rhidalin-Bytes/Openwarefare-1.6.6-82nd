...
main()
{
	...
	level.scr_ftag_fire_instant_unfreeze = getdvarx( "scr_ftag_fire_instant_unfreeze", "int", 0, 0, 1 );
...
onSpawnPlayer()
{
	...
	if ( self.freezeTag["frozen"] && self depthOfPlayerInWater() < 1 ) {
...
startHeatTransfer( frozenPlayer, triggerRadius )
{
	...
	if ( isDefined( self ) ) {
		...
		if ( totalPoints > 0 ) {
			...
			self thread giveUnfreezeScore( totalPoints );
		}
		self.freezeTag["transfer"] = false;
	}
...
addHealthPoint( budy, points )
{
	// Check if we have points defined
	if ( !isDefined( points ) )
		points = 1;
	...
	if ( i < self.freezeTag["unfreezebudies"].size ) {	
		self.freezeTag["unfreezebudies"][i].healthGiven += points;
	}			
}
...
playerFrozenDamaged( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime )
{
	// Check if we can instantly unfreeze a player with fire
	if ( level.scr_ftag_fire_instant_unfreeze == 0 )
		return;
		
	// Check if the player was damaged with fire
	if ( ( sMeansOfDeath == "MOD_GRENADE" && sWeapon == "molotov_mp" ) || ( sMeansOfDeath == "MOD_BURNED") ) {
		healthGiven = self.maxhealth - self.health;
		self.health = self.maxhealth;
		self addUnfreezingBudy( eAttacker );
		self thread addHealthPoint( eAttacker, healthGiven );
		eAttacker.freezeTag["healthgiven"] += healthGiven;
		
		totalPoints = int( eAttacker.freezeTag["healthgiven"] / level.unfreezeUnitsPerPoint );
		if ( totalPoints > 0 ) {
			eAttacker.freezeTag["healthgiven"] = eAttacker.freezeTag["healthgiven"] - ( totalPoints * level.unfreezeUnitsPerPoint );
			eAttacker thread giveUnfreezeScore( totalPoints );
		}		
	}	
}