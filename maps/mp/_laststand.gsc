...
initLastStand()
{
	...
	level.specialty_pistoldeath_no_weapons = getdvarx( "specialty_pistoldeath_no_weapons", "int", 0, 0, 1 );
...
PlayerLastStand( eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration )
{
	...
	for( i = 0; i < self.previousweaponslist.size; i++ )
	{
		if ( WeaponClass( self.previousweaponslist[i] ) == "pistol" ) 
		{
			...
			totalAmmoLeft = self getAmmoCount( self.laststandpistol );
			clipAmmoLeft = self getWeaponAmmoClip( self.laststandpistol );
			self.hadPistol = true;
			break;
		}
	}
	...
	self iprintln( "I'm here...");
	self takeallweapons();

	// Check if the player should get any weapon
	if ( level.specialty_pistoldeath_no_weapons == 0 ) {
		// the last stand pistol is given after the weaponslist is made
		if ( level.specialty_pistoldeath_check_pistol == 0 || ( self.hadPistol && totalAmmoLeft > 0 ) ) {
			self giveWeapon( self.laststandpistol );
			self switchToWeapon( self.laststandpistol );
		
			if ( totalAmmoLeft > 0 ) {
				self SetWeaponAmmoClip( self.laststandpistol, clipAmmoLeft );
				self SetWeaponAmmoStock(  self.laststandpistol, totalAmmoLeft - clipAmmoLeft );			
			} else {
		...
		self iprintln( "Player can have weapons...");
	} else {
		self iprintln( "Player can't have weapons...");
	}
...
player_being_revived( playerBeingRevived )
{
	self endon( "death" );
	self endon( "disconnect" );
	reviveTime = getdvarx( "revive_time_taken", "float", 2.0, 0.0, 10.0 );
